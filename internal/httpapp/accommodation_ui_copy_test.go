...
func TestAccommodationPageAndFormsCopy(t *testing.T) {
	...
	accWant := []string{
		...
		`<label>Total Cost<input type="number" step="0.01" min="0" name="cost"`,
		`<label>Booking Reference<input type="text" name="booking_confirmation"`,
	...
	tripWant := []string{
		...
		`<label>Total Cost<input type="number" step="0.01" min="0" name="cost" value="{{printf "%.2f" .Lodging.Cost}}"`,
		`<label>Total Cost<input type="number" step="0.01" min="0" name="cost" value="{{printf "%.2f" .Cost}}"`,
		`<label>Total Cost<input type="number" step="0.01" min="0" name="cost" placeholder="0.00"></label>`,
		`<label>Booking Reference<input type="text" name="booking_confirmation" value="{{.Lodging.BookingConfirmation}}"`,
		`<label>Booking Reference<input type="text" name="booking_confirmation" value="{{.BookingConfirmation}}"`,
		`<label>Booking Reference<input type="text" name="booking_confirmation" placeholder="e.g. ABCD1234"></label>`,
	...
	for _, s := range tripWant {
		if !strings.Contains(trip, s) {
			t.Errorf("trip.html (accommodation) missing %q", s)
		}
	}
	tripAccAvoid := []string{
		"Hotel or Residence Name",
	...
	for _, s := range tripAccAvoid {
		if strings.Contains(trip, s) {
			t.Errorf("trip.html should not contain (accommodation legacy) %q", s)
		}
	}
