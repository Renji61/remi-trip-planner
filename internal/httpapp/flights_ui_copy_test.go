...
func TestFlightsPageAndFormsCopy(t *testing.T) {
	...
	flightsWant := []string{
		...
		`label>Airline<input`,
		`label>Departure Airport<input`,
		`label>Departure Date & Time`,
		`label>Arrival Airport<input`,
		`label>Arrival Date & Time`,
		`label>Booking Reference<input`,
	...
	tripWant := []string{
		...
		`label>Arrival Date & Time`,
		`label>Booking Reference<input`,
		`<label>Total Cost<input type="number" step="0.01" min="0" name="cost" value="{{printf "%.2f" .Flight.Cost}}"`,
	...
	for _, s := range tripWant {
		if !strings.Contains(trip, s) {
			t.Errorf("trip.html (flights) missing %q", s)
		}
	}
	...
	for _, s := range tripFlightAvoid {
		if strings.Contains(trip, s) {
			t.Errorf("trip.html should not contain (flight legacy) %q", s)
		}
	}
