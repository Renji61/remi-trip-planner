...
func TestHighestStableReleaseTag(t *testing.T) {
	...
	if version.Compare(got, "1.50.0") != 0 {
		t.Fatalf("compare")
	}
}
...
func TestHighestSemverFromGitTagNames(t *testing.T) {
	names := []string{"v1.40.0", "v1.50.0", "v1.45.0", "not-a-version", "v2.0.0-rc1"}
...
func TestLatestUpstreamSemverTagsWinWhenReleasesStale(t *testing.T) {
	...
	if got != "1.50.0" {
		t.Fatalf("got %q want 1.50.0 (tags must lift above old release)", got)
	}
}
